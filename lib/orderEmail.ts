...
function itemsTable(items: OrderItem[], total: number): string {
  const rows = items
    .map(
      (it) => `
      <tr>
        <td style="padding:10px 0;border-bottom:1px solid #e7ddd2;color:${INK};font-size:14px;">
          ${esc(it.product.name)}
          <span style="color:${ROSE};font-size:12px;"> &nbsp;·&nbsp; ${it.quantity} × ${it.product.price} €</span>
        </td>
        <td style="padding:10px 0;border-bottom:1px solid #e7ddd2;color:${INK};font-size:14px;font-weight:600;text-align:right;white-space:nowrap;">
  ...
  return `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin:8px 0;">
      ${rows}
      <tr>
        <td style="padding:14px 0 0;color:${INK};font-size:16px;font-weight:700;">Celkom</td>
  ...
  return `<!doctype html><html><body style="margin:0;padding:0;background:${LINEN};">
    <div style="max-width:560px;margin:0 auto;padding:32px 20px;font-family:Arial,Helvetica,sans-serif;">
      <div style="background:${INK};padding:22px 28px;">
        <span style="color:${LINEN};font-size:22px;font-weight:700;letter-spacing:.5px;">Beli<span style="color:${LIME};">Wood</span></span>
      </div>
...
/** E-mail pre admina — kompletný prehľad objednávky. */
export function adminEmail(o: OrderPayload) {
  ...
  return {
    ...
    text:
      ...
      o.items.map((i) => `- ${i.product.name} (${i.quantity} × ${i.product.price} €) = ${i.product.price * i.quantity} €`).join("\n") +
      `\n\nCelkom: ${o.total} €` +
      (o.note?.trim() ? `\n\nPoznámka: ${o.note}` : ""),
  };
}
...
export function customerEmail(o: OrderPayload) {
  const inner = `
    ...
    <p style="margin:0 0 16px;color:${INK};font-size:15px;line-height:1.6;">
      Vašu objednávku sme úspešne prijali. Čoskoro vás budeme kontaktovať
